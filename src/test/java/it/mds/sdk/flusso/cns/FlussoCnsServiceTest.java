...
public class FlussoCnsServiceTest {

    final String CNS_ACN_CSV = "CNS_ACN.csv";
    ...
    @Autowired
    private ConfigurazioneFlussoCns configurazioneFlusso;
    @InjectMocks
    @Spy
    private FlussoCnsService flusso;
    @Mock
    ...
    private RecordDtoCns recordDto;
    private ConfigurazioneFlussoCns configurazione;
    ...
    void validazioneBlocchiTestModalitaT_KO() {
        ...
        given(validationEngine.validaFlussoBloccoConRegoleInteroFlussoCNS(anyList(), any(), anyString(), anyList(), anyInt())).willReturn(bloccoValidazione);
    ...
    void validazioneBlocchiTestModalitaT_KO2() {
        ...
        willThrow(new XSDNonSupportedException()).given(validationEngine).puliziaFileDatarootCns(any(), any(), any());
    ...
    void validazioneBlocchiTestModalitaT_OK() {
        ...
        ConfigurazioneFlussoCns configurazioneFlussoCns = new ConfigurazioneFlussoCns();
        when(spy(conf.getFlusso())).thenReturn(configurazioneFlussoCns.getFlusso());
        when(conf.getXmlOutput()).thenReturn(configurazioneFlusso.getXmlOutput());
        List<RecordDtoGenerico> list = List.of(recordDto);
        EsitoCDM esito = new EsitoCDM("abc", "cde");
        List<EsitoCDM> esitoCDMList = List.of(esito);
        ...
        willReturn("nomeFile").given(validationEngine).puliziaFileDatarootCns(any(), any(), any());
        given(validationEngine.validaFlussoBloccoConRegoleInteroFlussoCNS(anyList(), any(), anyString(), anyList(), anyInt())).willReturn(bloccoValidazione);
    ...
    void validaTracciatoTestEsitiCDM() {
        ...
        given(parser.parseTracciatoBloccoCDM(any())).willReturn(esitoCDMList);
