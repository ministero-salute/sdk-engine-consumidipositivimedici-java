...
class ParserTracciatoImplTest {
    private static final String FILE_TRACCIATO_CNS = "CNS.csv";
    ...
    void init() {
        ...
        Properties prop = loadPropertiesFromFile("config-flusso-cns-test.properties");
    ...
    void validaTracciatoOK() {
        ...
        File tracciato = new File(absolutePath + FileSystems.getDefault().getSeparator() + FILE_TRACCIATO_CNS);
    ...
    @Test
    void validaTracciatoBloccoCNS() throws SQLException {
        ...
        parserTracciato.parseTracciatoBloccoCDM(tracciato);
    }
    ...
    void integBLocco() {
        ...
        List<RecordDtoCns> r2 = list.stream().map(k -> (RecordDtoCns) k).collect(Collectors.toList());
    ...
    void scritturaNBlocchiSuXML() {
        ...
        List<RecordDtoCns> bloccoConv = blocco.stream().map(k -> (RecordDtoCns) k).collect(Collectors.toList());
        ...
        bloccoConv = blocco.stream().map(k -> (RecordDtoCns) k).collect(Collectors.toList());
